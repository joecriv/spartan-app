...
// Subtype rotation drag state (cooktops + sinks)
let rotatingItemId = null;
let rotateBaseAngle = 0;     // angle from center to mouse at drag start
let rotateBaseRotation = 0;  // s.rotation at drag start
...
function hitShape(mx, my) {
    for (let i = shapes.length-1; i >= 0; i--) {
        ...
        if (s.shapeType === 'l') {
            ...
            // Rotated subtype rect: inverse-rotate the click point around
            // the item center before doing the axis-aligned bounds check.
            if (s.rotation) {
...
function drawEdgeDatum(ed, storeKey, x1, y1, x2, y2, sel, labelOffset) {
    ...
    if (etype !== 'none') {
        ...
        if (letter) {
            ...
            ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
            // Repeat the single-letter code every ~20" along the edge so
            // it stays readable on B&W prints. Letter sits ON the line —
            // a thick white halo punches the line out behind each letter
            // for legibility.
            const STEP_IN = 20;
...
function drawShape(s, sel) {
    ...
    // Apply rotation transform for rotatable subtypes (sinks + cooktops).
    // The transform is restored at the end of drawShape via _rotApplied.
...
// Draws the rotation handle for sinks/cooktops. Called from render() after
// drawShape so it works regardless of which early-return path drawShape took.
function drawSubtypeRotateHandle(s) {
    ...
    ctx.strokeStyle = '#fff'; ctx.lineWidth = 1.5; ctx.stroke();
    ctx.beginPath();
    ctx.arc(hp.x, hp.y, 3, -Math.PI * 0.7, Math.PI * 0.85);
    ctx.strokeStyle = '#fff'; ctx.lineWidth = 1.2; ctx.stroke();
    ctx.restore();
}
// Returns the world position of the rotation handle for a rotatable subtype.
function subtypeRotateHandlePos(s) {
    const cxR = s.x + s.w/2, cyR = s.y + s.h/2;
    const rad = (s.rotation || 0) * Math.PI / 180;
    // Local handle position: just outside top-right corner (diagonal).
...
cv.addEventListener('mousedown', e => {
    ...
    // Rotation handle hit on currently-selected sink/cooktop
    if (selected !== null) {
...
function refreshToolPrompt() {
    ...
    if (!prompt) return;
    // Hide while popup active or once user has dismissed it (e.g. picked
    // an edge profile). Reset on tool change in setTool below.
